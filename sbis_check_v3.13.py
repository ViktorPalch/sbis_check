...
async def stoptopupcall(call: types.CallbackQuery):
    ...
    if expert in list_expert:
        namber_chat = (list_expert[expert][2])
...
async def scan_message(message: types.Message):
    ...
    sys.stdout = log
    print(f'Время: {datetime.datetime.now()} - username: {message.from_user.username}, Имя: {message.from_user.first_name}, Фамилия: {message.from_user.last_name}')
