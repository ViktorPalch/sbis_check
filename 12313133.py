            ...
            if str(sheet[row][10].value) not in s.keys():
                ...
                if len(str(s[sheet[row][10].value][1])) == 14 and len((s[sheet[row][10].value][1])) != 7 or len(
                    ...
                    d16 = datetime.timedelta(days=d16.day, hours=d16.hour, minutes=d16.minute, seconds=d16.second,
                                             microseconds=d16.microsecond)
                elif len(str(s[sheet[row][10].value][1])) >= 22 or len(str(s[sheet[row][10].value][1])) <= 23:
                ...
                elif len(str(sheet[row][14].value)) >= 20:
                    s14 = datetime.datetime.strptime(
                        ((str(sheet[row][14].value))[0] + ' ' + (str(sheet[row][14].value))[7:]), '%d %H:%M:%S.%f')
                    s14 = datetime.timedelta(days=s14.day, hours=s14.hour, minutes=s14.minute, seconds=s14.second,
                                             microseconds=s14.microsecond)

                else:
                    print(f'Тут ошибка {len(str(sheet[row][14].value))}, s14')
